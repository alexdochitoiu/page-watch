  ...
  async checkWatcher(url: string, rules: WatcherRule[]): Promise<WatcherCheckResult> {
    ...
    try {
      ...
      for (const strategy of strategies) {
        try {
          console.log(`🔄 [WATCHER-ENGINE] Trying navigation strategy: ${strategy.name} (timeout: ${strategy.timeout}ms)`);
          await page.goto(url, { 
          ...
          console.warn(`⚠️ [WATCHER-ENGINE] Strategy ${strategy.name} failed:`, error instanceof Error ? error.message : error);
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        console.log(`📊 [WATCHER-ENGINE] Rule ${rule.id} result (${ruleTime}ms):`, JSON.stringify(result, null, 2));
        ...
        if (i === 0) {
          ...
          if (prevRule.logicOperator === 'and') {
            ...
            console.log(`📊 [WATCHER-ENGINE] AND operation: ${previousSuccess} && ${result.passed} = ${overallSuccess}`);
          } else if (prevRule.logicOperator === 'or') {
            overallSuccess = overallSuccess || result.passed;
            console.log(`📊 [WATCHER-ENGINE] OR operation: ${previousSuccess} || ${result.passed} = ${overallSuccess}`);
          } else {
            console.log(`📊 [WATCHER-ENGINE] No logic operator, keeping previous success: ${overallSuccess}`);
          }
        }
      }

      const totalTime = Date.now() - startTime;
      console.log(`✅ [WATCHER-ENGINE] All rules evaluated in ${totalTime}ms. Overall success: ${overallSuccess}`);
      ...
      console.log(`📊 [WATCHER-ENGINE] Final result summary: ${finalResult.success ? 'SUCCESS' : 'FAILURE'}`);
      return finalResult;

    } catch (error) {
      const totalTime = Date.now() - startTime;
      console.error(`💥 [WATCHER-ENGINE] Error during watcher check after ${totalTime}ms:`, error);
      console.error(`💥 [WATCHER-ENGINE] Error stack:`, error instanceof Error ? error.stack : 'No stack trace');
      ...
      switch (operation) {
        ...
        case Operation.CONTAINS:
          ...
          if (elementCount === 0) {
            ...
            console.log(`📊 [RULE-EVAL] CONTAINS: "${actualValue}" contains "${value}" = ${result}`);
          }
          break;
        ...
        case Operation.GREATER_THAN:
          console.log(`🔍 [RULE-EVAL] GREATER_THAN: getting numeric content to compare with ${value}`);
          if (elementCount === 0) {
            ...
            console.log(`📊 [RULE-EVAL] GREATER_THAN: ${gtNumber} > ${gtTarget} = ${result} (from "${actualValue}")`);
          }
          break;
        ...
        case Operation.LESS_THAN:
          ...
          if (elementCount === 0) {
            ...
            console.log(`📊 [RULE-EVAL] LESS_THAN: ${ltNumber} < ${ltTarget} = ${result} (from "${actualValue}")`);
          }
          break;
          ...
          if (elementCount === 0) {
            ...
            if (value) {
              try {
                ...
                console.log(`📊 [RULE-EVAL] REGEX_MATCH: /${value}/.test("${actualValue}") = ${result}`);
              } catch (regexError) {
